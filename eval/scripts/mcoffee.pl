#!/usr/bin/env perl
#
# Runs T-Coffee in M-Coffee mode to compute the consensus of alignments
# created by several other methods. Alignments must be in Fasta format.
#
...
#  output is refalign.mcoffee.fsa
...
sub mcoffee {
  ...
  my $ref = shift;
  my $m = $ref, $ref = shift if $ref =~ /^-/;
  my $reftree = shift;

  $ref =~ /(.*)\./, my $base = $1;
  my @flist = grep -e $_, map "$base.$_.fsa", @methods;
  @flist > 1 or print(STDERR "mcoffee.pl: did not find alignments to combine!\n"), return{};
  my $out = "$base.mcoffee.fsa";

  require "${path}tcoffeecons.pl";
  tcoffeecons($m?$m:(), $ref, @flist, $reftree?$reftree:(), "-out=$out");
