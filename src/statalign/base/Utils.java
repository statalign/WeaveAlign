...
public class Utils{
   ...
   static public boolean calcFelsenWithCheck(double[] res, double[] fel1, double[][] prob1, double[] fel2, double[][] prob2) {
		double s;
		int i, j, len = res.length;
...
//		ErrorMessage.showPane(null, System.getProperty("java.class.path"), false);
	 ...
	 * @param tree The tree that contains the alignment. Needed for the Nexus format that needs
	 *             a name of the alignment (set to tree.title) the type of the alignment (either
	 *             nucleotide or protein, read from tree.substitutionModel) and the list of characters
	 *             in the substitution model (also read from tree.substitutionModel).
	 * @return String array containing the alignment in the prescribed format
	 */
   public static String[] alignmentTransformation(String[] s, String type, Tree tree){
...
//				newNames[leavesNumber] = newNames[leavesNumber].replaceAll("_", " ");
		...
		//	System.out.println("only leaves number: "+onlyLeavesAlignment.length+
			...
			returnAlignment[1] = "[TITLE: "+tree.title+"]";
			returnAlignment[2] = "";
			...
			returnAlignment[5] = "format interleave datatype="+tree.substitutionModel.getType()+"   gap=- symbols=\"";
			for(int i = 0; i < tree.substitutionModel.alphabet.length; i++){
				returnAlignment[5] += tree.substitutionModel.alphabet[i];
			}
			returnAlignment[5]+="\";";
			returnAlignment[6] = "";
