...
public class Vertex{
	...
	Vertex(Tree owner, double edgeLength, int[][] seq, String name){
		this.owner = owner;
		this.edgeLength = edgeLength;
	...
	void updateHmm3Matrix(){
		if(left != null && right != null) {
			...
			//	System.out.println("the reduced transition matrix is: "+owner.hmm3.preCalcRedTransMatrix(null, null));
		}
	}
			...
			for(j = 0; j <= childLen; j++) {
				...
				for(k = START+1; k < END; k++) {
					...
					if(previ >= 0 && prevj >= 0) {
						...
						if(previ == 0 && prevj == 0)
							tr = hmm2TransMatrix[START][k];
						else {
							for(tr = Utils.log0, prevk = START+1; prevk < END; prevk++)
								tr = Utils.logAdd(tr, probMatrix[previ][prevj][prevk]+hmm2TransMatrix[prevk][k]);
						}
						probMatrix[i][j][k] = Math.log(emissionProb)+tr;
					} else
						probMatrix[i][j][k] = Utils.log0;
	...
	double hmm2BackProp() {
		...
		while(prevk != END) {
			...
			double bp = Utils.log0;
			for(int bpPrevk = START; bpPrevk < END; bpPrevk++)
				bp = Utils.logAdd(bp, probMatrix[previ][prevj][bpPrevk]+hmm2TransMatrix[bpPrevk][k]);
			bp = probMatrix[previ][prevj][prevk]+hmm2TransMatrix[prevk][k]-bp;
	...
	double hmm2Align(){
		...
		for(k = END; k != START; k = prevk) {
			for(prevk = START; prevk < END; prevk++)
				prJump[prevk] = probMatrix[previ][prevj][prevk]+hmm2TransMatrix[prevk][k];
			prevk = Utils.logWeightedChoose(prJump, retVal);
	...
	public int countNameLengthSum(){
		return nameLengthSum = (left == null && right == null) ? 
				Math.min(10,name.indexOf(' ') == -1 ? name.length() : name.indexOf(' ')) + 2 : left.countNameLengthSum() + right.countNameLengthSum();
	}
	...
	double fastSwapWithUncle() {
		...
		while(p != null){
			...
			p = pf.prev; pf = pf.prev;
			t = tf.prev; tf = tf.prev;
			b = bf.prev; bf = bf.prev;
		}
		//for grandpa
		AlignColumn g = grandpa.last, gf = grandpa.last;
		p = parent.last; pf = parent.last;
...
//			  System.out.println(".");
			...
			g = gf.prev; gf = gf.prev;
			p = pf.prev; pf = pf.prev;
			u = uf.prev; uf = uf.prev;

		}
		// if there is a grand-grandpa...
		if(grandpa.parent != null){
			g = grandpa.last; gf = grandpa.last;
	...
	double alignAllWindows(){
		...
		p = last; //pf = last;
		l = left.last; lf = left.last;
		r = right.last; rf = right.last;
		...
		while(l != null){
			...
			//	    }
			l = lf.prev; lf = lf.prev;
			r = rf.prev; rf = rf.prev;
			if(l != null){
				p = new AlignColumn(p, true);
				length++;
				p.left = l; p.right = r;
				l.parent = p; l.orphan = false;
				r.parent = p; r.orphan = false;
