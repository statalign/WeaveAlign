...
public class Tree extends Stoppable {
	 ...
	 * drowing a novel ancestral substring for their parent vertex, the proposed new
	...
	public Tree(String[] sequences, String[] names, SubstitutionModel model, SubstitutionScore ss, String filename) throws StoppedException {
		...
		for(int i = 0; i < sequences.length; i++){
			...
			for(int j = 0; j < sequences[i].length(); j++){
				int sum = 0;
				for(int l = 0; l < ss.which[sequences[i].charAt(j)].length; l++){
					sum += ss.which[sequences[i].charAt(j)][l];
				}
				if(sum > 0.5){
					k++;
				}
			...
			for(int j = 0; j < sequences[i].length(); j++){
				...
				if(sum > 0.5){
					seq[i][k] = ss.which[sequences[i].charAt(j)];
					k++;
				}
			}
		...
		for(int i = 0; i < seq.length; i++){
			vertex[i] = new Vertex(this, 0.0, seq[i], names[i]);
		}
		// NJ main recursion
		int vnum = seq.length;
		Vertex newVert;
