...
public class LogLikelihoodTrace extends statalign.postprocess.Postprocess{
	...
	public void newSample(int no, int total) {
		double logLike = mcmc.tree.getLogLike();
		if(postprocessWrite)
		...
		if(sampling){
			try {
				...
				new ErrorMessage(mcmc.postprocMan.mainManager.frame," "+e.getLocalizedMessage(),true);
			}
		}
		else{
	...
	public void newStep() {
		LogLikelihoodTraceContainer container = new LogLikelihoodTraceContainer(
				mcmc.tree.getLogLike(), mcmc.burnin);
		if(list.size() < 300){
			list.add(container);
		} else {
			count++;
	...
	public void afterLastSample() {
		if(postprocessWrite)
			try {
				//  FileWriter fw = new FileWriter("output/"+mcmc.tree.title+".loglikelihood.txt", false);
