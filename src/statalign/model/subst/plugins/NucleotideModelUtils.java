...
import statalign.Jama.EigenvalueDecomposition;
import statalign.Jama.Matrix;
...
public class NucleotideModelUtils {
	 ...
	 */
	public static void numericalDiagonalisation(SubstitutionModel M, double Q[][]){
		/* Symmetricalise Q */
		for (int i = 0; i < Q.length; i++)
			for (int j = 0; j < i; j++)
				Q[j][i] = Q[i][j] = Math.sqrt(M.e[i] / M.e[j]) * Q[i][j];
