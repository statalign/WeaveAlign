...
public class MinRiskAnnotator {
	...
	private int minRiskMode = 1;		// 0: P(col), 1: P(col, state)
	...
	public MinRiskAnnotator(Tree tree, double[][] transMat, double[] initState, List<? extends SubstitutionModel> models) {
		this.tree = tree;
	...
	public MinRiskAnnotator(Tree tree, double[][] transMat, double[] initState, SubstitutionModel model, List<Double> rhos) {
		this.tree = tree;
		this.transMat = Utils.toLog(transMat);
		this.initState = Utils.toLog(initState);
	...
	public Column annotate(ColumnNetwork network, String[] sequences, String[] seqNames) {
		...
		System.out.println("MPD mode is "+(minRiskMode+1));
